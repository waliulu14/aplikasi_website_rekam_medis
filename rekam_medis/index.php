...
<div class="box">
    ...
    <form name="proses" method="POST">
        <div class="table-responsive">
            <table id="dataTableRm" class="display table-sm table table-striped table-bordered table-hover" style="width: 100%;">
                <thead>
                    <tr>
                        <th style="width: 1%; text-align: center;">No</th>
                        <th style="width: 15%; text-align: center;">Nama Pasien</th>
                        <th style="width: 15%; text-align: center;">Nama Dokter</th>
                        <th style="width: 15%; text-align: center;">Ruangan</th>
                        <th style="text-align: center;">Obat</th>
                        <th style="width: 15%; text-align: center;">Tanggal Periksa</th>
                        <th style="width: 100px; text-align: center;"><i class="glyphicon glyphicon-cog"></i> &nbsp; <input type="checkbox" id="select_all"></th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    $no = 1;
                    $sql_select_rm = mysqli_query($connect, "SELECT * FROM tb_rekam_medis JOIN tb_pasien ON tb_pasien.id_pasien = tb_rekam_medis.id_pasien JOIN tb_dokter ON tb_dokter.id_dokter = tb_rekam_medis.id_dokter JOIN tb_poliklinik ON tb_poliklinik.id_poli = tb_rekam_medis.id_poli");
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                $sub_total = 0;
                                $sql_select = mysqli_query($connect, "SELECT * FROM tb_rm_obat JOIN tb_obat ON tb_obat.id_obat = tb_rm_obat.id_obat  WHERE id_rm = '$row[id_rm]'");
                                while ($item = mysqli_fetch_array($sql_select)) {
                                    $sub_total += $item['harga'];
                                ?>
                                    <div>
                                        <div>- <?= $item['nama_obat']; ?>, <?= $item['catatan']; ?></div>
                                    </div>
                                <?php } ?>
                            </td>
